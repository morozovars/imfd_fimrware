...
{
  hdma_arr[inst][ch].Init.Mode = cfg->mode;
  ...
  hdma_arr[inst][ch].Init.Request = cfg->request;

  hdma_arr[inst][ch].Instance = (DMA_Channel_TypeDef *)channels_[inst][ch];
...
{
  return hal2code(HAL_DMA_RegisterCallback(&hdma_arr[inst][ch], (HAL_DMA_CallbackIDTypeDef) cb_id, (halCallbackFuncTypedef)cb));
}
